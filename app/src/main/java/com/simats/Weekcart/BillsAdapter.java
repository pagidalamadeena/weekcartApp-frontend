...
public class BillsAdapter extends RecyclerView.Adapter<BillsAdapter.BillViewHolder> {
    ...
    public void onBindViewHolder(@NonNull BillViewHolder holder, int position) {
        ...
        // Show only the date instead of "Order #ID"
        String displayDate = bill.getUploadedAt();
        if (displayDate != null && displayDate.contains("T")) {
            displayDate = displayDate.split("T")[0];
        }
        holder.tvBillId.setText(displayDate != null ? displayDate : "N/A");
        holder.tvBillDate.setVisibility(View.GONE); // Hide the smaller date field as it's now redundant
